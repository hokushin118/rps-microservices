...
class ScoreServiceV1ImplITSpec extends AbstractIntegrationTests implements EntityTests {
    ...
    def 'Testing caching functionality of the findById() method, scores should be cached'() {
        ...
        def actualScore = getCachedScoreDtoById(TEST_UUID, ScoreDto.class, SCORE_ADMIN_CACHE_NAME).get() as ScoreDto
        assert expectedScore.id.toString() == actualScore.id
        assert expectedScore.winner.name() == actualScore.winner

        and:
        "Im-memory cache contains $SCORE_ADMIN_CACHE_NAME cache"
        assert cacheManager.cacheNames.contains(SCORE_ADMIN_CACHE_NAME)
    }
    ...
    def 'Testing caching functionality of the all() method, scores should be cached'() {
        ...
        assert !isEmpty(cacheManager.getCache(SCORES_ADMIN_CACHE_NAME))
    }
    ...
    def 'Testing caching functionality of the findByWinner() method, scores should not be cached'() {
        ...
        assert isEmpty(cacheManager.getCache(SCORE_ADMIN_CACHE_NAME))
    }
}
