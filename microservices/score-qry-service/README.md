...
#### 2. Deploying the Score query microservice

To deploy the Score query microservice to Kubernetes, first deploy the microservice K8S service with the following command:
...
To check the state of the deployment, first forward the Score query microservice to your local environment with the following command:

```
     > kubectl port-forward svc/score-qry-service-svc 8084 -n rps-app-dev
```
